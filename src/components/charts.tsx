    ...
    <div className="h-72 w-full md:h-80">
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <Tooltip {...TOOLTIP} labelFormatter={(d) => shortDate(String(d))} formatter={(v, n) => [Number(v).toLocaleString("it-IT"), n === "specNet" ? "Speculativi" : "Commercial"]} />
          <Line type="monotone" dataKey="commNet" stroke="#71717a" strokeWidth={1.25} dot={false} isAnimationActive={false} />
          <Line type="monotone" dataKey="specNet" stroke="#d97757" strokeWidth={2} dot={false} isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
    ...
    <div className="h-64 w-full md:h-72">
      <ResponsiveContainer>
        <AreaChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          ...
          <CartesianGrid {...GRID} />
          <XAxis dataKey="index" {...AXIS} minTickGap={32} />
          ...
          <Tooltip {...TOOLTIP} labelFormatter={(i) => `Trade ${i}`} formatter={(v) => [Number(v).toLocaleString("it-IT", { minimumFractionDigits: 2, maximumFractionDigits: 2 }), "Capitale"]} />
    ...
    <div className="w-full" style={{ height }}>
      <ResponsiveContainer>
        <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
          <CartesianGrid {...GRID} />
          <XAxis dataKey="key" {...AXIS} interval={0} />
          <YAxis tickFormatter={compact} width={52} {...AXIS} />
          <ReferenceLine y={0} stroke="rgba(255,255,255,0.2)" />
          <Tooltip {...TOOLTIP} cursor={{ fill: "rgba(255,255,255,0.04)" }} formatter={(v) => [Number(v).toLocaleString("it-IT", { maximumFractionDigits: 2 }), label]} />
